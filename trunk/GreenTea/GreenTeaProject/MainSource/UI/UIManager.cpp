...
// Load a skin
UISkin* UIManager::LoadSkin(const tchar* sSkinName)
{
	if(NULL == sSkinName)
	{
		return NULL;
	...
	if(false == pSkin->Create(sSkinName))
	{
		pSkin->Release();
		pSkin = NULL;
	}
...
void UIManager::CreateChildrenWidget(GTTreeNode* pParentTreeNode, UIWidget* pParent)
{
	...
	GTTreeNode* pChildrenNode = pParentTreeNode->FindChild(PropertyFile::TagChildren);
	if(pChildrenNode)
	{
...
UIWidget* UIManager::LoadLayout(const tchar* szLayoutName)
{
	...
	GTTreeNode* pTreeNode = PropertyFile::Load(szLayoutName);
	if(NULL == pTreeNode)
	{
		return NULL;
	}
	...
	if(pLayoutRoot)
	{
		CreateWidget(pLayoutRoot, m_pRootWidget);
	}
