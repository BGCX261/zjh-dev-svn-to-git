...
const tchar* g_AttackAnimationName[4] =
{
...
int32 BSUnitBehaviour::OnInitialize(GTBehaviourInitParam* param)
{
...
int32 BSUnitBehaviour::OnStart(void)
{
	...
	m_pMainPlayer = SceneGameObjectManager::GetInstance()->FindGameObjectByName(BSPlayerController::m_szMainPlayName);
	if(m_pMainPlayer)
	{
	...
	if(GetPropertyState() != BSUnitBehaviour::State_Attack)
	{
		...
		if(nAnimationIndex >= 0)
		{
			...
			PlayAnimationByName(
				...
				GTObject_CallbackFunction_Set(BSUnitBehaviour::OnAnimationPlayingCompletion)
				);
			// Fire weapon
			FireWeapon();
		}
...
void BSUnitBehaviour::OnResetWeapon(void)
{
	...
	{
		m_pWeaponObject->SetVisible(false);
