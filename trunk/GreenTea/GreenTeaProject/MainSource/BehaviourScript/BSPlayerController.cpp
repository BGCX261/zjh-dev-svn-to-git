...
int32 BSPlayerController::OnInitialize(GTBehaviourInitParam* param)
{
	...
	if(m_pSprite)
	{		
		//m_pSprite->m_pSprite->PlayAnimationByName(CTEXT("RunUp"));
		//m_pSprite->m_pSprite->PlayAnimationByName(CTEXT("RunDown"));
		//m_pSprite->m_pSprite->PlayAnimationByName(CTEXT("RunLeft"));
		m_pSprite->m_pSprite->PlayAnimationByName(CTEXT("RunRight"), GSpriteAnimation::PlayMode_Once);
		m_pSprite->m_pSprite->StopAnimation();
	}
	// Cache BS objects
	m_pPathFinding = (BSPathFinding*)BSCLASSNAME_GET_COMPONENT(BSClassName::BSPathFinding);
	m_pSpriteMovement = (BSSpriteMovement*)BSCLASSNAME_GET_COMPONENT(BSClassName::BSSpriteMovement);
...
int32 BSPlayerController::OnStart(void)
{
	...
	GTGameObject* pStartObject = SceneGameObjectManager::GetInstance()->GetRootGameObject()->FindChild(CTEXT("StartPosition"));
	GTGameObject* pEndObject = SceneGameObjectManager::GetInstance()->GetRootGameObject()->FindChild(CTEXT("EndPosition"));
	if(pStartObject)
	{
