...
int32 BSTweener2D::OnInitialize(GTBehaviourInit* param)
{
...
void BSTweener2D::TweenUpdate(void)
{
	...
	if(m_pGameObject)
	{
		...
		long lDeltaTime = TimeManager::Inst()->GetTime() - m_lPrevTime;
		GTVector2 position = m_Tweener.Update(lDeltaTime);
		// Save current time
		m_lPrevTime = TimeManager::Inst()->GetTime();
