...
bool BSGameObjectGeneral::CreateFromPropertyTreeNode(GTTreeNode *pTreeNode)
{
	...
	// Collision
	GTTreeNode* pCollisionNode = pTreeNode->GetChild(GTPropertyFile::TagCollision);
	if(pCollisionNode)
		m_Collider.SetColliderSize(pCollisionNode->ValueToSize());
	else // Set default size
		m_Collider.SetColliderSize(GTSize(32.0f, 32.0f));

	// FollowParent
	GTTreeNode* pFollowParentNode = pTreeNode->GetChild(GTPropertyFile::TagPairValue);
...
int32 BSGameObjectGeneral::OnInitialize(GTBehaviourInit* param)
{
	if(NULL == param)
		return 0;
...
int32 BSGameObjectGeneral::OnStart(void)
{
	GTBehaviour::OnStart();
...
// Copy
bool BSGameObjectGeneral::Copy(GTObject* pObjectFrom)
{
	GTBehaviour::Copy(pObjectFrom);

	BSGameObjectGeneral* pSrc = (BSGameObjectGeneral*)pObjectFrom;
	DBG_ASSERT(pSrc);

	SetPropertyFollowParent(pSrc->GetPropertyFollowParent());
	m_Collider = pSrc->m_Collider;
