...
bool GameMainScene::ccTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
	...
	GTInputManager::GetInstance()->OnSingleClickEvent(UIWidget::Flag_ClickDown, GTPoint2(m_tBeginPos.x, m_tBeginPos.y));
...
void GameMainScene::ccTouchMoved(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
	...
	// Notify input manager
	GTInputManager::GetInstance()->OnMoveEvent(0, GTPoint2(diff.x, diff.y));
...
void GameMainScene::ccTouchEnded(CCTouch* pTouch, CCEvent* pEvent)
{
	...
	m_tBeginPos = pTouch->getLocation();
	// Notify input manager
	GTInputManager::GetInstance()->OnSingleClickEvent(UIWidget::Flag_ClickUp, GTPoint2(m_tBeginPos.x, m_tBeginPos.y));
}
