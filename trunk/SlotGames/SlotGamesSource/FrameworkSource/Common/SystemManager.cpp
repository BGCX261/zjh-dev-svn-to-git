...
int32 SystemManager::OnInitialize(void)
{
	...
	g_fSystemManagerStartTime = (float)GOSys::GetTime() / 1000.0f;//g_fSystemManagerStartTime = (float)timeGetTime() / 1000.0f;
...
{
	float fCurrentTime = ((float)GOSys::GetTime()/1000.0f) - g_fSystemManagerStartTime;//float fCurrentTime = ((float)timeGetTime()/1000.0f) - g_fSystemManagerStartTime;
