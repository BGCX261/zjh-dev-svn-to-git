...
bool SceneManager::IntializeGameObjects()
{
	...
	// Get game object list in the map
	...
	if(pObjects)
	{
		...
		CCARRAY_FOREACH(pObjects, pObj)
		{
			...
			if(pContent)
			{
				// Create a new child game object for pRootGameObject
				...
				{					
					...
					// Query the type
					cocos2d::CCString *sType = (cocos2d::CCString*)pContent->objectForKey(std::string(GTPropertyFile::TagType));
					...
					if(NULL != sZOrder)
					{
						pChild->SetZOrder(sZOrder->intValue());
					}
					...
					if(sProperty)
					{
						...
						GTGameObjectManager::Inst()->InitializeGameObject(pChild, sProperty->getCString());
					}
					else
					{
